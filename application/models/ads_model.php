...
class Ads_model extends CI_Model
{
	...
	{
		$query=$this->db->query("select * from `xz_ads` where `title` like '%{$str}%' order by sort asc");
		return $query->result_array();
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('xz_ads');
	}
	
	//get_where
	public function  user_get_where($arr)
	{
		$query=$this->db->get_where('xz_ads',$arr);
		return $query->result_array();
	}
