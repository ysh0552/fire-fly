...
class Down_model extends CI_Model
{
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('xz_cn_down');
	}

	public function user_select_index_limit($index)
	{
		$this->db->select('*');
	...
	{
		$query=$this->db->get_where('xz_cn_down',$arr);
		return $query->result_array();
	}
	...
	{
		$query=$this->db->query("select * from `xz_down` where `id`<{$id} order by `id` desc limit 1");
		return $query->result_array();
	}
	
	public function user_next($id)
	{
		$query=$this->db->query("select * from `xz_down` where `id`>{$id} order by `id` asc limit 1");
		return $query->result_array();
	}
	
	public function user_search($str)
	{
		$query=$this->db->query("select * from `xz_down` where `title` like '%{$str}%' ");
		return $query->result_array();
	}
